var finances = [
  ...
  var totalProfitLosses = finances.reduce((acc, val) => acc + val[1], 0);
  ...
  var greatestIncrease = finances.reduce((acc, val, index, array) => {
    if (index === 0) return acc;
    var diff = val[1] - array[index - 1][1];
    if (diff > acc.amount) {
      return { amount: diff, date: val[0] };
    } else {
      return acc;
    }
  }, { amount: -Infinity });
  
  var greatestDecrease = finances.reduce((acc, val, index, array) => {
    if (index === 0) return acc;
    var diff = val[1] - array[index - 1][1];
    if (diff < acc.amount) {
      return { amount: diff, date: val[0] };
